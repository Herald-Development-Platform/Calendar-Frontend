      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <Button
            ...
            role="combobox"
            aria-expanded={open}
            className="flex w-[50px] items-center justify-center px-0"
          >
            {/* {value
            ? locations?.find((location) => location.name === value)?.name
            : "Select Locations..."} */}
            <ChevronsUpDown strokeWidth={1.3} className="size-4 shrink-0 opacity-50" />
          </Button>
        </PopoverTrigger>
        <PopoverContent className="z-[999] w-[500px] p-0" align="end">
          <Command>
              ...
              <CommandGroup>
                {locations?.map((location, index) => (
                  ...
                  >
                    ...
                    <div className="ml-auto hidden h-[26px] w-[26px] cursor-pointer items-center justify-center rounded-full bg-white group-hover:flex">
                      <FaPlus></FaPlus>
                    </div>
                  </CommandItem>
                ))}
