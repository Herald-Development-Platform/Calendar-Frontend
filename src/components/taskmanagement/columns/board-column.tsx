...
import { useCreateTask, useGetTaskByColumn } from "@/services/api/taskManagement/taskApi";
import toast from "react-hot-toast";
import { EditColumnDialog } from "./edit-column-dialog";
import { useDeleteColumn, useUpdateColumn } from "@/services/api/taskManagement/columnsApi";
    ...
    createTask(
      { title, column: column._id },
      {
        onSuccess: data => {
          ...
          // Get current cache
          const existing = queryClient.getQueryData<{ data: ITask[] }>(queryKey);
                ...
                <DropdownMenu open={dropdownOpen} onOpenChange={setDropdownOpen}>
                  ...
                  >
                    <DropdownMenuItem
                      ...
                      className="mx-1 my-1 flex cursor-pointer items-center gap-2 rounded-md text-sm hover:bg-gray-50"
                    >
                    ...
                    <DropdownMenuItem
                      ...
                      className="mx-1 my-1 flex cursor-pointer items-center gap-2 rounded-md text-sm text-red-600 hover:bg-red-50"
                    >
        ...
        >
          {isEmpty && !showAddCard ? (
            <div className="text-center text-gray-500">
              ...
              <p className="mt-1 text-xs text-gray-400">Add your first task to get started</p>
            </div>
          ) : (
            <>
              {!disableDnD ? (
                  ...
                  >
                    ...
                    >
                      {activeTasks.map((task, index) => (
                        <div
                          key={task?._id}
                          className="duration-200 animate-in slide-in-from-top-2"
